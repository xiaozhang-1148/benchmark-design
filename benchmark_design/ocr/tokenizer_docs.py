...
def write_taxonomy_rules_md(output_path: Path) -> None:
    ...
    lines = [
        ...
        *priority_lines,
        ...
        "",
        "- **english tokens** — single ASCII letters (`a`–`z`, `A`–`Z`) not matched as dictionary commands.",
        "- **digit tokens** — single ASCII digits `0`–`9`.",
        "- **greek tokens** — Greek letter commands such as `\\alpha`, `\\Gamma`, `\\varphi`.",
        "- **special symbol tokens** — arrows, misc symbols, spacing commands such as `\\therefore`,",
        "  and other dictionary entries classified as symbols (excluding greek / structural / operator / grouping sets).",
        "- **operator tokens** — `+`, `-`, `=`, `<`, `>`, `*`, `/`, and relation / large-operator commands.",
        "- **grouping tokens** — visible delimiters: parentheses, brackets, and `\\left` / `\\right`, plus literal `\\{` / `\\}`.",
        "- **structural tokens** — TeX scope delimiters `{` `}`, `^`, `_`, fractions, roots, sums, integrals, limits, matrices / environments.",
...
def write_metric_definitions_md(output_path: Path) -> None:
    lines = [
        ...
        "",
        "## AST depth (structure-forest max depth)",
        "",
        "Computed by the unified structure forest (`structure_forest.py`) on each expression's token list.",
        "",
        "- **AST depth** — maximum tree depth of structure nodes in the forest for that expression.",
...
def write_data_schema_md(output_path: Path) -> None:
    lines = [
        ...
        "| `ast_depth` | int | Structure-forest max tree depth. |",
...
def write_known_limitations_md(output_path: Path) -> None:
    lines = [
        ...
        "``{group}`` arguments). Structure-forest metrics always run; low parse success rates are",
