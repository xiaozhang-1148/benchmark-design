...
def test_classify_token_examples() -> None:
    assert classify_token("a") is TokenCategory.LATIN_VARIABLE
    ...
    assert classify_token(r"\alpha") is TokenCategory.SPECIAL_SYMBOL
...
def test_compute_ocr_token_taxonomy_fixture(sample_benchmark_dir: Path) -> None:
    ...
    assert rows["latin variable tokens"] == (10, pytest.approx(0.25))
    ...
    assert rows["grouping tokens"] == (10, pytest.approx(0.25))
    assert rows["structural tokens"] == (4, pytest.approx(0.1))
...
def test_write_token_taxonomy_report(sample_benchmark_dir: Path, tmp_path: Path) -> None:
    ...
    assert "latin variable tokens,10," in csv_text
