...
def test_page_level_latex_export_smoke(tmp_path: Path) -> None:
    ...
    assert (output_dir / "figures" / "page_ast_scale.png").is_file()
    assert (output_dir / "figures" / "page_tokens_structure.png").is_file()
    assert not (output_dir / "figures" / "fig6_1_page_ast_trees.png").exists()
    assert not (output_dir / "figures" / "fig6_4_9_10_tokens_structure.png").exists()
    assert not (output_dir / "figures" / "fig6_page_token_structure_scale.png").exists()
    assert not (output_dir / "figures" / "fig6_page_ast_scale.png").exists()
    assert (output_dir / "plot_data" / "page_tokens_structure_plot_data.csv").is_file()
    assert not (output_dir / "plot_data" / "fig6_1_page_ast_trees_plot_data.csv").exists()
